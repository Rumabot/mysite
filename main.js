let fadeInTarget = document.querySelectorAll(".typing-demo"); //".fade-in"セレクタの要素をすべて取得、その要素を変数fadeInTargetに代入、リスト
window.addEventListener("scroll", () => { //スクロールイベントを監視、スクロールが発生した場合は{}内の処理実行
  for (let i = 0; i < fadeInTarget.length; i++){ //リストの要素を一つずつ処理
    ...
    if (scroll > offset - windowHeight + 150) { //要素が画面内に表示されたかを判定
      fadeInTarget[i].classList.add("typing-demo"); //画面内に表示された場合は、"scroll-in"クラスを追加
    }
    else{
        fadeInTarget[i].classList.remove("typing-demo")
    }
  }
});
...
gsap.fromTo(b, {
  ...
  innerHTML:'<p><sub>MEMBER</sub> '+(i+1)+'</p>',
  width:'100%',
  height:'100%',
